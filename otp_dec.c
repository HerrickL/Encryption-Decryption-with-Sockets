/***********************************************************************************************************
* Name: Program 4 - opt_dec.c
* Class: CS 344-400
* Written By: L. Herrick 
* Description:  program connects to opt_dec_d and requests one-time-pad style encryption of a user-input
* 		message.  Takes a ciphertext filename, key filename, and port number as arguments. When
* 		opt_dec recieves the message text from opt_enc_d, it outputs to stdout.   
*********************************************************************************************************/
#include <sys/types.h>
...
int main(int argc, char *argv[]){
	...
	int maxSize = 2000;
	char message[maxSize];		//array will hold encrypted message
	char key[maxSize];		//array will hold key
	char endmess[maxSize];		//array will hold decrypted message

	//error check usage
	if(argc < 4){
		printf("Usage: opt_dec encrypt_fileName key_fileName portNum [&]\n");
		exit(1);
	}
